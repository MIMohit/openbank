...
The Financial-grade API (FAPI) 2.0 Security Profile reached Final status in February 2025 and mandates sender-constrained access tokens via mTLS or DPoP. Sender-constraining removes an entire class of Open Banking attacks — network replay and reuse of a token stolen without its key — and commodity identity infrastructure now implements it. This reframes a question that the Zero Trust literature for financial APIs has not answered with measurement: once a deployment is already FAPI 2.0-correct, what does an additional layer of continuous, context-bound enforcement actually add, at what operational cost, and against which adversaries? We build a reproducible, fully synthetic Open Banking testbed — a Basiq-isomorphic Accredited Data Recipient API, a Keycloak identity provider configured to the FAPI 2.0 DPoP profile, and a Zero Trust controller performing per-request device-binding, telemetry collection, rule-based risk scoring, and adaptive `ALLOW`/`CHALLENGE`/`DENY` decisions through an external policy engine — and evaluate three configurations under an identical executable attacker suite: a plain bearer baseline (B0), a FAPI 2.0/DPoP baseline (B1), and the continuous-enforcement system (P).

The measured answer is narrow and, in its most consequential part, negative. B1 alone takes network replay and token-theft-without-key from certain success to zero (30/30 to 0/30; risk difference −1.00) for a median cost of 0.60 ms; continuous enforcement adds nothing against them. Of the three attacks B1 cannot stop, continuous enforcement changes the outcome of exactly one — account takeover from a newly enrolled device, reduced from 30/30 to 11/30 (risk difference −0.633, 95% CI [−0.806, −0.461]) — and leaves device-resident key abuse and volume abuse untouched at 30/30. Reading the controller's own per-request records shows that even the one success is fragile: the adversary is refused only while one of two transient signals happens to fire, both artefacts of sliding-window bookkeeping rather than properties of the adversary, and the single signal the adversary cannot forge is priced 0.35 against a 0.40 challenge threshold and so cannot change a decision alone. We therefore measure the same takeover played by an adversary who paces below the rate rule and mimics the victim's self-reported context — capabilities the threat model already grants — and it succeeds on 30 of 30 attempts. A one-step threshold recalibration identified by a sweep over the recorded risk scores blocks both the naive and the adaptive adversary completely, at identical measured friction on legitimate traffic, which locates the failure in the operating point rather than in the signal for this adversary; for device-resident abuse no threshold works, because the only rule addressing it fires on 100% of legitimate requests at a median of 36 times its own threshold.

The cost is substantial and misattributed by design intuition. Continuous enforcement adds 6.2 ms at the median and 27.6 ms at p99 (Cliff's δ = 0.58, large), 11.5 percentage points of controller CPU and 5.0 MiB of memory, and refuses 7.4% of legitimate requests — 97.3% of those whose context drifts. Cryptography is not the cost: DPoP proof verification takes 0.19 ms, about 1% of a request, while a single round trip to the out-of-process policy engine takes 9.39 ms, 55% of the request and more than the backend call it guards. An ablation attributes the entire usability cost to the contextual component and shows no single component reduces the takeover below 0.967. We release the testbed, attack suite, raw records and analysis pipeline. Our contribution is not a new mechanism but a measured characterization of the security/cost boundary of continuous Zero Trust enforcement over a modern financial-API baseline, including a negative result about the robustness of its one demonstrated benefit and a validated remedy for it.
...
**A5 does not discriminate between configurations and should not be read as evidence for any of them.** It is refused in all three, including B0, because object-level ownership is enforced at the resource server and not by the enforcement layer under test. We report it for completeness of the taxonomy, and note that a system whose BOLA defence lives entirely downstream of the enforcement layer gets no credit for that defence.
